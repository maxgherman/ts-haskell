...
import { monad as monadBase } from '@control/monad/plain-reader';
import { doRepeat } from '@control/common/monad';

const monad = monadBase();

describe('PlainReader monad', () => {
    const a = (x) => x + 1;
    const b = (x) => x * 2;
    const f = r => x => r + x;
    const g = r => x => r * x;
    ...
    describe('Monad first law (Left identity): return a >>= k = k a', () => {
        ...
        it('with compose', () => {
            ...
            expect(result1(10)).toEqual(result2(10));
            expect(result1(10)).toBe(30);
        });
    });
    ...
    describe('Monad - Applicative first relationship: pure = return', () => {
        ...
        it('for non arg', () => {
            expect(monad.return(5)(10)).toEqual(monad.pure(5)(10));
            expect(monad.pure(5)(10)).toBe(5);
        });

        it('for empty array', () => {
            expect(monad.return(null)(10)).toBe(monad.pure(null)(10));
        });
    });

    describe('do - notation', () => {
        const run = (r) => (x) => Math.max(r, x);
        const start = 10;
        ...
        it('Monad first law (Left identity): do { x′ <- return x; f x′ } = do { f x }', () => {
            ...
            expect(result1(1)).toBe(10);
            expect(result1(20)).toBe(20);    
        });
        ...
        it('Monad second law (Right identity): do { x <- m; return x } = do { m }', () => {
            ...
            expect(result1(1)).toBe(result2(1));
            expect(result1(1)).toBe(a(1));
            expect(result1(20)).toBe(a(20));
            expect(result1(20)).toBe(21);    
        });
    ...
    it(`Monad third law (Associativity):
            ...
            expect(result1(1)).toBe(result(1));
            expect(result(1)).toBe(result2(1));
            expect(result(1)).toBe(3);
            expect(result(20)).toBe(820);
            expect(result1(20)).toBe(result(20));
        });
});