import { $null, head, List, nil, tail } from './list'
...
export type Case<T> = NonNullable<[Pattern, (...params: never[]) => T][]>
...
const matchPattern = <T>(p: Pattern, list: List<T>): PatterMatchResult<T> => {
    ...
    return p.reduce((acc, _, index) => matchPatternEntry(acc, index, p.length), {
        value: [],
        rest: nil(),
        list,
        result: false,
    ...
    (list: List<T>): K => {
        ...
        for (const pattern of caseOf) {
            ...
            if (matchResult.result) {
                const result = (
                    matchResult.rest ? [...matchResult.value, matchResult.rest] : [...matchResult.value]
                ) as never[]
