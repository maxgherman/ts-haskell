...
import { functor, ListFunctor } from './functor'
import { foldable, ListFoldable } from './foldable'
import { ListBox, $null, head, tail, nil, cons } from './list'
import { MinBox1 } from 'data/kind'

export interface ListTraversable<T> extends Traversable, ListFunctor, ListFoldable<T> {
...
const sequenceA = <A>(app: Applicative, tfa: ListBox<MinBox1<A>>): MinBox1<ListBox<A>> => {
    ...
    return app.liftA2((x: A) => (xs: ListBox<A>) => cons(x)(xs), head(tfa), sequenceA(app, tail(tfa)))
}
