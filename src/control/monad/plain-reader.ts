...
export interface IPlainReaderMonad<T> extends IMonad<IsPlainReader> {
    ...
    lift<A, B>(
        fab: PlainReaderBox<T, Application<A, B>>,
        fa: PlainReaderBox<T, A>): PlainReaderBox<T, B>
    '<*>'<A, B>(
        fab: PlainReaderBox<T, Application<A, B>>,
        fa: PlainReaderBox<T, A>): PlainReaderBox<T, B>
    liftA<A, B>(f: Application<A, B>, fa: PlainReaderBox<T, A>): PlainReaderBox<T, B>
    liftA2<A, B, C, X extends Application2<A, B, C>>(
        abc: X, fa: PlainReaderBox<T, A>,
        fb: PlainReaderBox<T, B>): PlainReaderBox<T, C>
    liftA3<A, B, C, D>(
        f: Application3<A, B, C, D>,
        fa: PlainReaderBox<T, A>,
        fb: PlainReaderBox<T, B>,
        fc: PlainReaderBox<T, C>): PlainReaderBox<T, D>
    ...
    '<**>'<A, B>(
        fa: PlainReaderBox<T, A>,
        fab: PlainReaderBox<T, Application<A, B>>): PlainReaderBox<T, B>

    '>>='<A, B>(
        ma: PlainReaderBox<T, A>,
        action: Application<A, PlainReaderBox<T, B>>): PlainReaderBox<T, B>
...
const implementation = {
    '>>='<R, A, B>(
        ma: PlainReaderBox<R, A>,
        action: Application<A, PlainReaderBox<R, B>>): PlainReaderBox<R, B> {
