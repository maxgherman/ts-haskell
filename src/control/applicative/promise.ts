...
export interface IPlainArrayApplicative extends IApplicative<IsPromise> {
    ...
    liftA2<A, B, C, X extends Application2<A, B, C>>(abc: X, fa: PromiseBox<A>, fb: PromiseBox<B>): PromiseBox<C>;
    liftA3<A, B, C, D>(f: Application3<A, B, C, D>, fa: PromiseBox<A>, fb: PromiseBox<B>, fc: PromiseBox<C>): PromiseBox<D>; 
...
const lift = <A, B>(fab: PromiseBox<Application<A, B>>, fa: PromiseBox<A>): PromiseBox<B> => {
    fab = fab || (Promise.resolve(identity) as Promise<Application<A, B>>);
