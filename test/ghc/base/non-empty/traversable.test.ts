...
tap.test('NonEmpty traversable', async (t) => {
    t.test('sequenceA', async (t) => {
        const tfa = fromList(listOf(just(1), just(2))) as NonEmptyBox<MaybeBox<number>>
        ...
        const tfa2 = fromList(listOf(just(1), nothing<number>())) as NonEmptyBox<MaybeBox<number>>
    ...
    t.test('traverse', async (t) => {
        const ne = fromList(listOf(1, 2)) as NonEmptyBox<number>
